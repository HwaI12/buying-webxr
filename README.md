...
- **画像ベースアンケート**: WebVRで描画したショーケース配置をスクリーンショット撮影し、画像と質問を用いたアンケート形式で評価を収集
- **視線追跡・行動ログなし**: 簡易評価実験として主観評価のみを収集
- **WebVR活用**: 実店舗では困難な複数配置の比較が容易、統制された環境での配置パターン生成
- **科学的手法**: 被験者内計画による統計的検証
...
WebXRシステムで各色配置パターンを描画し、スクリーンショットを撮影します：

1. **開発サーバーを起動**: `npm run dev`
2. **ブラウザでアクセス**: `http://localhost:5173`
3. **各パターンを表示**: 10個のパターンを順に表示
4. **スクリーンショット撮影**: ブラウザのスクリーンショット機能（F12 → Command+Shift+P → "Capture screenshot"など）を使用
5. **画像を保存**: 各パターンの画像を適切な名前で保存（例: `pattern-01-monochrome-white.png`）

### 2. アンケート実施の流れ

撮影したスクリーンショット画像を用いてアンケートを実施：

1. **同意取得**: 実験参加への同意
2. **基本情報**: 年齢、性別、色覚に関する情報（任意）
3. **説明**: 回答方法の説明
4. **練習試行**（任意）: 回答形式に慣れるための練習
5. **本試行**: 10個のパターン画像を順に提示（ランダム順序）
6. **各パターンの評価**:
   - パターン画像を表示
   - 購買意欲（7件法リッカート尺度）
   - 魅力度（7件法リッカート尺度）
   - 見やすさ（7件法リッカート尺度）
   - 自由記述（任意）
7. **事後質問**: 最も良いパターン、最も良くないパターン、ランキングなど

### 3. アンケートツールの選択肢

スクリーンショット画像を用いたアンケートは、以下のようなツールで実施可能：

- **Google Forms**: 画像挿入可能、7件法リッカート尺度対応
- **Microsoft Forms**: 同様の機能
- **SurveyMonkey / Qualtrics**: より高度なアンケート機能
- **紙ベース**: 印刷した画像を用いた紙のアンケート

### 4. データの記録と整理

アンケート結果を統計解析用のCSV形式で整理：

```csv
participantId,patternId,patternName,trialOrder,purchaseIntent,attractiveness,clarity,freeComment
P001,pattern-01-monochrome-white,単色（白）,1,3,2,5,"シンプルだが刺激が少ない"
P001,pattern-08-warm-dominant,暖色優位,2,6,6,4,"色が鮮やかで魅力的"
...
```
...
- **WebXR**: A-Frame 1.5.0（3Dショーケース配置の描画用）
- **3D 描画**: Three.js
- **フロントエンド**: Vanilla JavaScript (ES6 Modules)
- **ビルド**: Vite
- **サーバー**: Express
- **データ収集**: スクリーンショット画像 + 外部アンケートツール（Google Forms等）
...
- 色配置パターン評価実験への方向転換
- **画像ベースアンケートの実験設計**: VR画面のスクリーンショットを撮影し、画像と質問で評価を収集
- 10個の色配置パターンを定義（単色、多色、グラデーション、対称性など）
- データ構造の整理とファイル分割（環境設定、商品カタログ、実験設定、パターン定義）
- 統計解析手法の明確化（被験者内計画、対応のある分散分析、多重比較）
